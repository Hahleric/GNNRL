...
class Experiment(object):
    ...
    def start(self):
        ...
        while episode < 20:
            ...
            cache_efficiency_list.append(np.mean(episode_cache_efficiencies))
            episode += 1

        # 绘制每个 episode 最后 100 个 step 的平均 cache efficiency
        plt.figure(figsize=(10, 5))
        plt.plot(range(len(cache_efficiency_list)), cache_efficiency_list, marker='o')
        plt.xlabel('Episode')
        plt.ylabel('Average Cache Efficiency (last 100 steps)')
        plt.title('Average Cache Efficiency per Episode (last 100 steps)')
        plt.grid(True)
        plt.show()
