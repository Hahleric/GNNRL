...
from utils.data_preprocess import create_rating_matrix, get_ratings, create_top_100_rating_matrix, get_user_rated_movies, get_top_100_popular_movies
import numpy as np
import torch
...
if __name__ == '__main__':
    ...
    movies = get_top_100_popular_movies(data_set_path)
    ...
    env = environment.Environment(40,movies, user_ratings)
    agent = GCNAgent.GCNAgent(cache_size, 32)
    reward, cache_efficiency, request_delay = mini_batch_train(env, agent, 30, 100, 32, request_data, v2i_rate, v2i_rate_mbs, [i for i in range(user_ratings[0])], request_data)
