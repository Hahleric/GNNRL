...
class HeteroDotProductPredictor(nn.Module):
    def forward(self, graph, h, etype):
        with graph.local_scope():
            ...
            return graph, graph.edges[etype].data['score']
...
class BaseGraphModel(nn.Module):
    ...
    def forward(self, graph_pos, graph_neg):
        ...
        graph, score_pos = self.predictor(graph_pos, h, 'rate')
        _, score_neg = self.predictor(graph_neg, h, 'rate')
        return graph, score_pos, score_neg
    ...
    def get_score_by_user_item(self, h, user, item):
        user_embed = h['user'][user]
        item_embed = h['item'][item]
        scores = torch.mm(user_embed, item_embed.t())
        return scores
